...
public class HideoutPatcher(ILogger<HideoutPatcher> logger, DatabaseService databaseService, ConfigLoader configLoader) : IOnLoad
{
    ...
    private readonly HideoutConfig _hideoutConfig = configLoader.Load<HideoutConfig>(HideoutConfig.FileName);

    public Task OnLoad()
    {
        if (!_hideoutConfig.EnableHideoutConfig) {
            return Task.CompletedTask;
        }

        ApplyAreaOverrides();
        PatchFoundInRaid();
        PatchBitcoinFarm(_hideoutConfig.BitcoinFarm);
