...
public class ConfigLoader(ILogger<ConfigLoader> logger, ModHelper modHelper)
{
    ...
    public T Load<T>(string filename) where T : new()
    {
        ...
        if (!File.Exists(path))
        {
            logger.LogError("[RZServerManager] {File} not found : using defaults.", filename);
            var def = new T();
    ...
    public IEnumerable<T> LoadAll<T>(string subfolder, IEnumerable<string>? exclude = null) where T : new()
    {
        ...
        {
            logger.LogError("[RZServerManager] Folder '{Dir}' not found.", dir);
            return [];
        }
        ...
        return Directory.GetFiles(dir, "*.json")
            ...
            .Select<string, T?>(path =>
            {
                try
                {
                    ...
                    if (result is null)
                        logger.LogError("[RZServerManager] Could not deserialize '{File}' : skipping.", path);
                    return result;
                }
                catch (Exception ex)
                {
                    logger.LogError("[RZServerManager] Failed to load '{File}' : {Msg} : skipping.", path, ex.Message);
                    return default;
                }
            })
