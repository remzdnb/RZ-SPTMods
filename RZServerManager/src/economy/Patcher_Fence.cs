...
) : IOnLoad
{
    ...
    public Task OnLoad()
    {
        ...
        if (_masterConfig.EnableSupplyConfig &&
            ...
            _supplyConfig.RestockTimes.TryGetValue(Traders.FENCE, out var fenceSeconds))
        {
        ...
        if (!_masterConfig.EnableFenceConfig) {
        ...
        {
            var fenceDbAssort = databaseService.GetTrader(Traders.FENCE)?.Assort;
            if (fenceDbAssort is not null)
            {
                foreach (var item in _fenceConfig.DefaultItemPoolAdditions)
                {
                    var rootItemAndChildren = assortHelper.CreateRootItem(item.Tpl);
    ...
    private void InjectCustomOffers()
    {
        ...
        for (var i = 0; i < _fenceConfig.CustomItemCount; i++)
        {
            ...
            var items = assortHelper.CreateRootItem(offer.Tpl, offer.StackSize, resolveChildren: true);
    ...
    private static class GenerateFenceAssortsPostFix
    {
        ...
        {
            if (_masterConfig != null && _masterConfig.EnableFenceConfig && _fenceConfig != null && _fenceConfig.EnableCustomItemPool)
            {
            ...
            if (_masterConfig != null && _masterConfig.EnableSupplyConfig && _restockSeconds.HasValue && _fenceService is not null)
            {
