...
) : IOnLoad
{
    ...
    private void ClearDefaultAssorts()
    {
        ...
        if (_masterConfig.EnableDevLogs) {
            logger.LogInformation("[RZCustomEconomy] DefaultTrades: all trader assorts cleared.");
        }
    }
    ...
    private void ApplyBlacklist(DefaultTradesConfig config)
    {
        if (config.Blacklist.Count == 0)
            return;

        var blacklistTpls = config.Blacklist.ToHashSet(StringComparer.OrdinalIgnoreCase);
        ...
        if (_masterConfig.EnableDevLogs)
            logger.LogInformation("[RZCustomEconomy] DefaultTrades/Blacklist: {Count} item(s) removed from trader assorts.", removed);
    }
    ...
    private void ApplyPriceMultipliers(DefaultTradesConfig config)
    {
        if (config.PriceMultipliers.Count == 0)
            return;
        ...
        foreach (var (traderId, multiplier) in config.PriceMultipliers)
        {
            if (multiplier == 1.0)
                continue;
