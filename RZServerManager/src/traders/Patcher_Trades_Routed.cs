...
public class Patcher_Trades_Routed(
    ILogger<Patcher_Trades_Routed> logger,
...
) : IOnLoad
{
    public Task OnLoad()
    {
        ...
        if (masterConfig.EnableDevLogs)
        {
            logger.LogInformation("[RZCustomEconomy] {Routed} auto-routed, {Skipped} skipped.{RouteAll}", routed, skipped, routedTradesConfig.ForceRouteAll ? " [ForceRouteAll ON]" : "");
        }
    ...
    {
        var vanillaPath = System.IO.Path.Combine(modHelper.GetAbsolutePathToModFolder(Assembly.GetExecutingAssembly()), "extras", "vanilla_items.json");
