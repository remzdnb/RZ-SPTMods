...
) : IOnLoad
{
    public Task OnLoad()
    {
        ...
        HarmonyPatch.Configs         = configLoader
            .LoadAll<ProfileConfig>("profiles", Assembly.GetExecutingAssembly())
            .Where(p => p.Enabled)
            .ToList();
        ...
        // Used to conditionally skip prestige reward mails during ApplyPrestige loop.
        harmony.Patch(
        ...
        logger.LogInformation("[RZCustomProfiles] Harmony hooks registered.");
        return Task.CompletedTask;
    }
}
