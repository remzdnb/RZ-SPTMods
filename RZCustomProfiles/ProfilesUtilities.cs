...
)
{
    public void PatchSide(TemplateSide? side, string sideName, ProfileConfig config)
    {
        ...
        ReplacePockets(side, sideName, config);

        ApplyHideoutLevels(side, sideName, config);
        MaxPlayerLevel(side, config);
        MaxSkills(side, config);
    ...
    private void AddStashItems(TemplateSide side, string sideName, ProfileConfig config)
    {
        ...
        var startingItemsCfg = config.StartingItems;
    ...
    private void ReplacePockets(TemplateSide side, string sideName, ProfileConfig config)
    {
        if (config.Pockets == 0) {
            return;
        }

        if (!MasterConfig.Pockets.TryGetValue(config.Pockets, out var tpl))
        {
            logger.LogWarning("[RZCustomProfiles] Unknown Pockets index '{Index}' — skipping.", config.Pockets);
            return;
        }

        var pockets = side.Character?.Inventory?.Items?.Where(i => i.SlotId == "Pockets").ToList();
        if (pockets is null || pockets.Count == 0)
        {
            logger.LogWarning("[RZCustomProfiles] {Side} Pockets not found — skipping.", sideName);
            return;
        }
    ...
    private void MaxPlayerLevel(TemplateSide side, ProfileConfig config)
    {
        if (!config.MaxLevel) {
            return;
        }
    ...
    private static void MaxSkills(TemplateSide side, ProfileConfig config)
    {
        if (!config.MaxSkills) {
            return;
        }
        ...
        const float maxSkillLevel = 5100f;
        foreach (var skill in skills) {
            skill.Progress = maxSkillLevel;
        }
    }
    ...
    private void ApplyTradersLoyalty(TemplateSide side, string sideName, ProfileConfig config)
    {
        ...
        {
            tradersInfo[new MongoId(traderId)] = new TraderInfo
            {
                Standing = traderConfig.Standing,
                SalesSum = traderConfig.SalesSum,
                Unlocked = traderConfig.Unlocked,
            };
        }
    }
