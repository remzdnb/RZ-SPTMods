...
public class ConfigLoader(ILogger<ConfigLoader> logger, ModHelper modHelper)
{
    ...
    public T Load<T>(string filename, Assembly callerAssembly) where T : new()
    {
        ...
        if (!File.Exists(path))
        {
            logger.LogError("[RZShared] {File} not found in '{Dir}' : using defaults.", filename, modDir);
            var def = new T();
    ...
    public IEnumerable<T> LoadAll<T>(string subfolder, Assembly callerAssembly) where T : new()
    {
        ...
        {
            logger.LogWarning("[RZShared] Folder '{Dir}' not found.", dir);
            return [];
        }
        ...
        return Directory.GetFiles(dir, "*.json")
            .Select(path =>
            {
                ...
                if (result is null) {
                    logger.LogWarning("[RZShared] Could not deserialize '{File}' - skipping.", path);
                }
